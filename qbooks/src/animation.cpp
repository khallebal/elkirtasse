...
void View::animChargeLevelTow()
{
    ...
    if (threeD==false){
     ...
     emit labelInfoChanged(currentItem->toolTip());
}
...
void View::animChargePixmape(int level,int index,int indexParent)
{
    ...
    if(level==1){
       ...
       emit labelInfoChanged(trUtf8("قائمة المجموعات"));
...
void View::ThreeDItemOpenBook()
{
    ...
    if(i>itemCurant)
    {
        ...
        if (!currentItem->data(0).isNull()){
           emit openBook( currentItem->data(0).toString(),currentItem->toolTip(),
                          currentItem->data(2).toString(),0);
        }
    }
...
void View::chargeRecent()
{
    ...
    emit labelInfoChanged(trUtf8("قائمة الكتب السابقة"));
    if (threeD==false){
        isThreeD=false;
...
void View::comboItemOpenBook()
{
    ...
    if (!currentItem->data(0).isNull()){
              emit openBook( currentItem->data(0).toString(), currentItem->toolTip(),
                             currentItem->data(2).toString(),currentItem->data(1).toInt());
    }
}
