...
dff->setTextValue(trUtf8("مجموعة جديدة"));
  ...
  if (text.isEmpty()){ return;}
    ...
    QVariant newId;
    newId=Utils::genirateId(ui->treeWidgetBooks);
qDebug()<<newId;
    itemNew->setText(0,text);
        itemNew->setIcon(0,icon);
...
void DialogConfigBooks::on_toolButtonUpdatBk_clicked()
{
    ...
    if (!item->data(1,1).isNull()){
        ...
        if( Utils::saveBookInfo(BookName,title,authur,betaka,pathCostm)==true)
        {
...
void DialogConfigBooks::on_buttonBox_accepted()
{
    ...
    QDir dir(pathCostm);
  if (  Utils::treeSaveGroupe(ui->treeWidgetBooks)==true){
...
//تحميل القائمة الى الشجرة
 ...
 //اضافة قسم جديد فارغ
   ...
   //اضافة مجموعة جديدة فارغة
       ...
       QVariant   newId=Utils::genirateId(ui->treeWidgetBooks);
       ...
       QString subdir;
         ...
         foreach ( subdir, dirImage.entryList(QDir::AllDirs | QDir::NoDotAndDotDot |
                 ...
                 QHash<QString,QString>hash=Utils::loadBookInfo(newBkPath+"/"+subdir);

                 QString title=hash.value("title");
                 QString author=hash.value("author");
                 QString bitka=hash.value("title");
...
void DialogConfigBooks::on_treeWidgetCurGroup_itemActivated(QTreeWidgetItem *item, int )
{
   on_treeWidgetCurGroup_itemClicked(item, 0 );
}

void DialogConfigBooks::on_treeWidgetCurGroup_itemClicked(QTreeWidgetItem *item, int )
{
