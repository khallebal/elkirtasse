...
void TabBook::loadSettings()
{
    ...
    if(pdfDoc)
    pdfDoc->setCostmColor(settings.value("isPdfColor",false).toBool(),m_WebColorBack);
...
loadPdfPlugin();
    if(pdfDoc){
    verticalLayoutPdf->addWidget(pdfDoc->PdfWidgetScroll());
...
void TabBook::setPdfPage(int index, QRectF rectF)
{
    if(!pdfDoc)return;
    ...
    if(m_isPdf&& pdfDoc){
        bool ok;
          int i = QInputDialog::getInt(this, trUtf8("الذهاب الى صفحة"),
                                       trUtf8("Enter number:"), pdfDoc->pageCurent(),
                                       1, pdfDoc->pagesCount(), 1, &ok);
          if (ok)
            setPdfPage(i);
          return;
    }
        ...
        QString id=item->data(0,1).toString().section(";",1,1);
         ...
         qreal rx=item->data(0,1).toString().section(";",2,2).toDouble();
          qreal ry=item->data(0,1).toString().section(";",5,5).toDouble();
             setPdfPage(id.toInt(),QRectF(rx*1000,ry*1000,5,5));
  treeViewFahras->setCurrentItem(item);
        return;
    }
...
//! open book---*فتح الكتب--------------------------------------------------------
int TabBook:: openDataBase(const QString &name,const QString &title,const QString &Autor,int position)
{
    ...
    if(m_bookPath.isEmpty())
        return DOC_FILE_NOEXIST;
    QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));

    if(name.startsWith("pdfs")&&name.endsWith("pdf")){
        if(!pdfDoc){
             QApplication::restoreOverrideCursor();
            return PDF_PLUG_NOEXIST;
        }
        if (pdfDoc->setDocument(m_bookPath)) {
            pdfDoc->loadToc(treeViewFahras);
            QList<QAction *>list;
            pdfDoc->setListActions(list<<mActionNext<<mActionPrev<<mActionFirst<<mActionLast);
            //   scalPdf=1.0;
...
int TabBook::getBookPos() const
  {
    if(m_isPdf&&pdfDoc)return pdfDoc->pageCurent();
...
void TabBook::serchInPdf(const QString &txt)
{
 ...
 //   pdfDoc->setEnsureVisible(rectF);
    emit pagePartChanged(trUtf8(" كتاب : ")+m_title+trUtf8(".  المؤلف : ")+m_author+
                         trUtf8("الصفحة :")+QString::number(pdfDoc->pageCurent()));
}
